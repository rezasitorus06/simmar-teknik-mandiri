...
<form method="POST" action="{{ $product->exists ? route('admin.products.update', $product) : route('admin.products.store') }}" enctype="multipart/form-data" class="mt-8 space-y-6 rounded-[26px] border border-[#d5e4f0] bg-white p-6 shadow-sm md:p-8">
    @csrf
    ...
    <div class="grid gap-5 md:grid-cols-2">
        ...
        <div>
            <label for="subcategory" class="mb-2 block text-sm font-medium text-ink">Subkategori</label>
            <select id="subcategory" name="subcategory" class="field" data-selected="{{ old('subcategory', $product->subcategory) }}">
                <option value="">Pilih subkategori</option>
            </select>
            @error('subcategory')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>
        <div>
    ...
    <label class="flex items-center gap-3 text-sm text-ink">
        <input type="checkbox" name="is_featured" value="1" @checked(old('is_featured', $product->is_featured)) class="h-4 w-4 accent-[#096dcc]">
        Tampilkan sebagai produk unggulan
    </label>
    <button class="primary-cta rounded-full px-6 py-3 text-sm font-bold">Simpan produk</button>
    <script>
        window.catalogCategories = @json($categories);
    </script>
</form></main>
</body>
</html>
