...
<header class="border-b border-[#dfe5dc] bg-white/80 backdrop-blur-sm">
    <div class="container-page flex h-20 items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-4">
            ...
            <form method="POST" action="{{ route('logout') }}">@csrf<button class="text-sm font-bold text-moss transition hover:text-ink">Keluar</button></form>
        </div>
    </div>
</header>
<main class="container-page py-12">
    ...
    @if (session('status'))
        <div class="mt-6 border border-[#b8d29f] bg-[#e7f3d2] px-4 py-3 text-sm text-moss">{{ session('status') }}</div>
    @endif

    <div class="mt-8 overflow-hidden rounded-[22px] border border-[#dfe5dc] bg-white shadow-sm">
        <div class="overflow-x-auto">
            <table class="w-full min-w-[650px] text-left text-sm">
                <thead class="border-b border-[#e2e6dc] bg-[#faf9f5] text-xs uppercase tracking-wider text-[#7c8981]">
                    <tr>
                        <th class="px-5 py-4">Produk</th>
                        <th class="px-5 py-4">Kategori</th>
                        <th class="px-5 py-4">Media</th>
                    ...
                    @forelse ($products as $product)
                        <tr>
                            ...
                            <td class="px-5 py-4 text-right">
                                <a href="{{ route('admin.products.edit', $product) }}" class="font-bold text-moss">Edit</a>
                                <form method="POST" action="{{ route('admin.products.destroy', $product) }}" class="ml-4 inline" onsubmit="return confirm('Hapus produk ini?')">
                                    @csrf
                                    @method('DELETE')
