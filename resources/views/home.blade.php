...
<main>
    <section class="relative overflow-hidden bg-[#d8e6e8] py-20 md:py-28">
        <div class="container-page grid items-end gap-12 md:grid-cols-[1.05fr_.95fr]">
            <div class="relative z-10"><p class="eyebrow text-moss">Simmar Teknik Mandiri</p><h1 class="mt-5 max-w-2xl text-5xl font-semibold leading-[.98] tracking-tight text-ink md:text-7xl">Ukur aliran dengan <span class="text-moss">lebih pasti.</span></h1><p class="mt-7 max-w-md text-base leading-7 text-[#52605a]">Distributor water meter dan flow meter untuk memastikan setiap aliran air terukur, terkontrol, dan siap mendukung operasional Anda.</p><a href="#produk" class="mt-9 inline-flex items-center gap-3 rounded-full bg-ink px-6 py-3.5 text-sm font-bold text-white transition hover:bg-moss">Lihat produk <span aria-hidden="true">&darr;</span></a></div>
            <div class="relative min-h-[300px] overflow-hidden rounded-[2px] bg-[#8eb8c0] md:min-h-[410px]"><img src="https://images.unsplash.com/photo-1581092160607-ee22621dd758?auto=format&fit=crop&w=1000&q=85" alt="Peralatan teknis untuk pengukuran aliran" class="absolute inset-0 h-full w-full object-cover mix-blend-multiply opacity-90"><div class="absolute bottom-5 left-5 rounded-full bg-lime px-4 py-2 text-xs font-bold text-ink">Akurasi untuk operasi</div></div>
        </div>
        <div class="pointer-events-none absolute -right-16 -top-16 h-60 w-60 rounded-full border-[32px] border-white/30"></div>
    </section>

    <section id="tentang" class="container-page grid gap-10 py-24 md:grid-cols-[.7fr_1.3fr] md:py-32"><p class="eyebrow text-moss">01 &nbsp; Tentang Simmar</p><div><h2 class="max-w-3xl text-3xl font-semibold leading-tight md:text-5xl">Solusi pengukuran untuk aliran yang <span class="text-moss">lebih terkendali.</span></h2><p class="mt-7 max-w-2xl leading-7 text-[#66716b]">Simmar Teknik Mandiri membantu bisnis dan kontraktor menemukan perangkat pengukuran yang tepat. Kami menyediakan produk untuk jaringan air bersih, proses industri, gedung, dan sistem utilitas.</p></div></section>

    <section id="produk" class="border-y border-[#dfe5dc] bg-[#faf9f5] py-20 md:py-24"><div class="container-page"><div class="flex items-end justify-between gap-5"><div><p class="eyebrow text-moss">02 &nbsp; Produk pilihan</p><h2 class="mt-3 text-3xl font-semibold md:text-4xl">Water meter & flow meter</h2></div><span class="text-sm text-[#7b857f]">{{ $products->count() }} produk</span></div><div class="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        @forelse ($products as $product)
            <article class="product-card group overflow-hidden"><div class="relative aspect-[4/3] overflow-hidden bg-[#dce9d6]">
